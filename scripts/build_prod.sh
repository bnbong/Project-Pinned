...
post17 = Post.objects.create(
user=User.objects.filter(username='박종윤').first(), 
landmark=Landmark.objects.filter(name='남대문갈치조림골목').first(), 
title='서울의 숨은 명소, 남대문갈치조림골목에서의 특별한 경험', 
content='안녕하세요! 오늘은 서울 남대문에 위치한 갈치조림골목에 대한 후기를 남겨볼게요. 이곳은 서울의 복잡한 길목에 위치한 작은 골목이지만, 방문자들에게는 큰 행복과 만족감을 줍니다. 갈치조림이라는 이름답게 이곳에서는 신선한 갈치를 이용한 다양한 요리를 맛볼 수 있어요. 갈치조림, 갈치구이 등 갈치 요리의 진수를 느낄 수 있었어요. 특히 갈치조림은 매콤한 양념과 갈치의 부드러운 맛이 조화를 이루며 입맛을 사로잡았습니다. 또한 골목길을 걸으며 볼 수 있는 오래된 상점들과 주민들의 친절한 모습도 인상적이었습니다. 이곳을 방문하면, 화려한 서울 도심에서 벗어나 작은 골목길의 소박하면서도 따뜻한 매력을 느낄 수 있어요. 서울에 방문하신다면 꼭 한 번 들려보시는 것을 추천합니다!')
post18 = Post.objects.create(user=User.objects.filter(username='허재원').first(), landmark=Landmark.objects.filter(name='남대문로낚시용품거리').first(), title='낚시 애호가의 천국, 남대문로낚시용품거리에서의 잊지 못할 경험', content='안녕하세요! 오늘은 낚시용품 애호가들 사이에서 잘 알려진 명소인 남대문로낚시용품거리에 대한 이야기를 해보려고 합니다. 이 곳은 이름에서 알 수 있듯이 낚시용품을 주로 판매하는 곳이에요. 가게마다 그 종류가 조금씩 다르지만 기본적으로 민물낚시용품부터 바다낚시용품까지 다양하게 구비하고 있습니다. 특히, 이곳의 물건들은 품질이 좋은 것으로 유명하며, 가격 또한 합리적인 편입니다. 이 때문에 전국의 낚시 애호가들이 이곳을 찾아와 낚시용품을 구입합니다. 또한, 가게 주인분들의 친절함은 덤이죠. 제가 낚시에 대해 아는 것이 그리 많지 않았는데도, 친절하게 설명해주시며 저에게 적합한 낚시 용품을 추천해주셨습니다. 이외에도 남대문로낚시용품거리는 위치적인 이점 또한 가지고 있습니다. 서울 중심에 위치해 있어 대중교통을 이용하여 편리하게 방문할 수 있고, 주변의 다른 관광 명소들과 연계된 관광을 즐길 수 있습니다. 낚시를 좋아하시는 분이라면 꼭 한 번 방문해보시는 것을 추천드립니다. 단순히 낚시용품을 구입하는 것을 넘어서, 다양한 낚시 용품에 대한 정보를 얻을 수 있으며, 동호인들과의 소통의 장을 만나볼 수 있습니다. 이곳에서의 경험은 분명히 여러분의 낚시 취미를 한층 더 풍요롭게 만들어 줄 것입니다.');"
fi